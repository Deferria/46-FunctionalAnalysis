...
\section{弱拓扑}
    ...
    \subsection{弱拓扑与弱*拓扑}
        ...
        \begin{thm}
            ...
            设 \(H\) 是一个可分的 Hilbert 空间, \(\{e_n\}_{n=1}^{\infty}\) 是 \(H\) 的一个标准正交基. 则 \(\{e_n\}_{n=1}^{\infty}\wlim 0\).
        \end{thm}
        ...
        \begin{prf}
            ...
            则它可以写作
            \[\prod_{\card (B)} [-1,1]\]
            根据 Tychonoff 定理, 这是一个紧拓扑空间. 将 \(X\) 上的泛函限制在 \(B\) 上, 我们只需要证明它是紧的. 根据紧拓扑空间的闭子集是紧的, 我们只需证明它是闭的. 将 \(f\) 延拓到整个空间上, 利用齐次性: \(\forall\,x\in X, F(x) = \norm{x} f\qty(\frac{x}{\norm{x}})\). 则 \(F\) 是有界线性泛函且 \(\norm{F}\leq\norm{f}\leq 1\). 从而 \(F\in X^*\). 这就说明了 \(X^*\) 的单位闭球在弱*拓扑下是紧的.
        \end{prf}
    ...
    \subsection{【实分析】Lp 空间}
        ...
        \begin{cxmp}
            ...
            这是因为 \(\Lp[1](X)\) 是可分的, 但 \(\Lp[\infty](X)\) 是不可分的: 取 \(X=[0,1]\), 则区间 \([0,\lambda]\) 的特征函数 \(\{\chi_{[0,\lambda]}: \lambda\in[0,1]\}\) 是 \(\Lp[\infty](X)\) 中的一族元素, 且任意两个元素之间的距离都是 1, 从而 \(\Lp[\infty](X)\) 中存在一个不可数的 1-分离子集, 从而 \(\Lp[\infty](X)\) 是不可分的.
        \end{cxmp}
        ...
        \begin{rmk}
            ...
            \begin{center}
                \begin{tabular}{c|cccc}
                    ...
                    可分性 & \checkmark & \checkmark & \checkmark & \texttimes\\
                    自反性 & \texttimes & \checkmark & \checkmark & \texttimes\\
                    单位球的一致凸性 & \texttimes & \checkmark & \checkmark & \texttimes\\
                    \bottomrule
                \end{tabular}
            \end{center}
            这也就解释了为什么同样是Lp空间, L2 空间非常优美, 但 L1 空间却臭名昭著.
        \end{rmk}
