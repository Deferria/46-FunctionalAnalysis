...
\section{线性泛函分析基础}
    ...
    \subsection{【点集拓扑】Baire 纲定理}
        ...
        \begin{thm}
            ...
            Baire 空间本身一定是\hyperref[dfn:second-category]{第二纲的}.
        \end{thm}
...
\section{弱拓扑}
    ...
    \subsection{弱拓扑与弱*拓扑}
        ...
        \begin{crl}
            ...
            这是因为可以建立从 Hilbert 空间到其\hyperref[dfn:dual-space]{对偶空间}的同构, 从而 Hilbert 空间的单位闭球在弱拓扑下也是\hyperref[dfn:compact]{紧的}.
        \end{crl}

        \begin{thm}
            {Goldstine 定理}
            [Goldstine's Theorem]
            设 \(X\) 是赋范线性空间, 则 \(X\) 的单位闭球 \(\overline{B}_X\) 的典范嵌入的像 \(J_X(\overline{B}_X)\) 在 \(X^{**}\) 的单位闭球 \(\overline{B}_{X^{**}}\) 中是弱*稠密的.\\
        ...
        \begin{prf}
            ...
            (\(\impliedby\)): 设存在 \(\xi\in\overline{B}_{X^{**}}\setminus(J_X(\overline{B}_X))^{w^*} \). 则这二者可以通过开集分离. 设 \(U\) 是弱*开集, \(U\cap (J_X(\overline{B}_X))^{w^*} =\varnothing\), \(\xi\in U\). 则
        ...
        \begin{ins}
            ...
            \begin{center}
                ...
                \end{tabular}
            \end{center}
            这里有一个关键差异, 在 Lebesgue 测度下, \(\Lp[1]([0,1])\) 不是任何空间的对偶空间, 这一点将在之后看到, \bysorry. 所以它不存在弱*拓扑.
        \end{ins}
...
\section{Banach 空间上的谱论}
    ...
    \subsection{紧致算子}
        ...
        \begin{ppt}
            ...
            若 $T\in\cpt(X,X)$, $S,U\in\mathcal{B}(X,X)$, 则 $U\circ T\in\cpt(X,X)$, $T\circ S\in\cpt(X,X)$.
        \end{ppt}

        \begin{rmk}
            [Dedicatia]
            这说明了在 $\blo(X,X)$ 中, $\cpt(X,X)$ 是一个双边理想. 这也是我们在后续定义 Fredholm 算子时的一个重要性质.
        \end{rmk}
        ...
        \begin{rmk}
            [Dedicatia]
            但是, 一般的 Banach 空间中的紧算子不一定能被有限秩算子逼近. 具备这种性质的 Banach 空间一般称为有限逼近性质的空间.
        \end{rmk}
    ...
    \subsection{Hilbert 空间上的算子}
        ...
        \begin{dfn}
            {正规算子}
            [Normal Operator]
            设 $H$ 是 Hilbert 空间, $T: H \to H$ 是一个有界线性算子. 定义 $T$ 是一个正规算子当且仅当 \(T^* T = T T^*\).
        \end{dfn}
